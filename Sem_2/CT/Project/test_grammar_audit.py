...
Two test categories:
  accept_tests  – inputs that are VALID per the PDF grammar  → parser must NOT error
  reject_tests  – inputs that are INVALID per the PDF grammar → parser MUST error
"""
import sys
from parser import parse

# ── Tests that the parser must ACCEPT ────────────────────────────────────────
accept_tests = {
    # ── unit ──
    "empty_program": "",
    "only_vardef": "int x;",
...
# ── Tests that the parser must REJECT (not in the PDF grammar) ───────────────
reject_tests = {
    # varDef: comma-separated multiple declarations — NOT in PDF
    "var_multi":       "int a, b, c; void main() {}",
    "var_multi_array": "int a, v[5], b; void main() {}",

    # arrayDecl: expression as size — NOT in PDF (only CT_INT? allowed)
...
print("=== ACCEPT tests (must parse without error) ===")
for name, src in accept_tests.items():
    try:
        parse(src)
        print(f"  PASS {name}")
        passed += 1
    except SystemExit:
        print(f"  FAIL {name}  ← should have been accepted")
        failed += 1

print("\n=== REJECT tests (must be rejected by the parser) ===")
for name, src in reject_tests.items():
    try:
        parse(src)
        # parser did NOT raise → this is wrong, it should have rejected it
